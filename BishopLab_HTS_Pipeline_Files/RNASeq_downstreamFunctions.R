DGE_pipeline <- function(
  ...
  if ("circos" %in% toRun) {
    ...
    if (species == "human") {
      ...
      print("Mouse!")
      cytobandURL <- "http://hgdownload.cse.ucsc.edu/goldenpath/mm10/database/cytoBandIdeo.txt.gz"
    }
    ...
    if (species == "human") {
      ...
      circos.initializeWithIdeogram(cytoband = cytoBand, chromosome.index = paste0("chr", c(1:19, "X", "Y")))
    }
...
DTEE_pipeline <- function(
  ...
  if (is.null(dds)) {
    ...
    resList[["dds"]] <- dds
    resList[['colData']] <- as.data.frame(colData(dds))
    if (! returnDataOnly) {
      save(dds, file = file.path(outdir, "dds.RData"))
    }
  }
